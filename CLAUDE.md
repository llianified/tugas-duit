...
Produksi memakai **AWS EC2** untuk Next.js production server, **Nginx** sebagai reverse proxy,
service systemd `tugas-duit`, dan **Neon PostgreSQL**. Origin produksinya
`https://littleoni.fun`; webhook Telegram produksinya
`https://littleoni.fun/api/telegram/webhook`. Render hanya menjalankan `redirect-server.js`
sebagai redirect legacy sementara ke origin produksi, bukan menjalankan aplikasi utama.
...
- **Deploy produksi berjalan di EC2.** Jalankan migrasi lewat `pnpm db:migrate` sebelum
  `pnpm build`, lalu restart service systemd `tugas-duit` dan pastikan `/api/health` sehat.
  Jangan jalankan migrasi dari service redirect legacy Render. Migrasi gagal berarti deployment
  harus dihentikan agar kode tidak live di atas skema yang belum siap.
