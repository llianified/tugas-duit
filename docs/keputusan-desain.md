...
| Lama tontonan minimum ditegakkan di jalur klaim klien, BUKAN di jalur postback | Dua mode, dua otoritas, dan tidak boleh dua-duanya menghakimi satu tayangan. Saat gerbang postback mati, yang mencetak pass adalah laporan dari perangkat — dan laporan itu satu-satunya yang perlu diperiksa ulang, jadi di situlah lantai waktunya berdiri. Saat gerbangnya menyala, yang mencetak pass adalah konfirmasi berbayar dari Monetag; menambahkan lantai waktu di sana berarti membuang impresi yang uangnya SUDAH masuk, dan lebih buruk lagi: kalau penyedia menembakkan `impression` begitu kreatifnya render, lantai apa pun akan menolak SELURUH tiket. Karena itu `settleAdPostback` sengaja tidak memeriksanya. | `adWatchTooShort` di `claimAdTicket`; test `ADS-DB-15` |
| `adsMinWatchSeconds` lahir 3 detik, dan angka itu memang belum cukup menutup "tap iklan lalu back" | 3 detik adalah lantai lama `MIN_WATCH_MS` apa adanya, yang selama ini hanya MENCATAT sinyal lalu tetap menerbitkan tiket. Dibawa jadi bawaan supaya menyalakan penegakannya tidak menghanguskan satu pun tiket yang selama ini sah — deploy-nya netral. Angka yang benar-benar memblokir hampir pasti lebih tinggi, tapi ia tidak boleh ditebak: jendelanya ikut memuat waktu memuat SDK (`waitForShow`, sampai 8 detik), jadi ia tidak sama dengan durasi kreatifnya. Pilih dari sebaran `ready_at - created_at` yang sudah tercatat, setel sedikit di bawah persentil 5. Terlalu tinggi = tiket berhenti terbit sementara impresinya tetap dibayar penyedia. | `adsMinWatchSeconds` di `ECONOMY_FIELDS`; migrasi `0042` |
...
| `arcadeEnabled` bawaannya **0**, padahal fitur lain lahir menyala | Turbo Reward lahir menyala karena ia cuma mengubah komunikasi. Arena mengubah berapa Rupiah yang keluar, jadi menyalakannya adalah keputusan ekonomi yang diambil di panel, bukan efek samping sebuah deploy. Snapshot `HISTORIC` di `domain/economy/economy-config.test.ts` yang menjaga bawaannya tidak bergeser diam-diam. | `DEFAULT_ECONOMY_CONFIG`; test `ARCADE-1` |
