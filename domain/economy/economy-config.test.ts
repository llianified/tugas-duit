...
const HISTORIC: EconomyConfig = {
  ...
  missionHardTarget: 5, missionHardReward: 2,
  ...
  missionWhatsappChannelReward: 1, missionTiktokFollowReward: 1, missionRacelyReward: 1,
...
describe('invarian setelan panel yang baru dipindah dari kode', () => {
  ...
  it.each([
    ...
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.errors[rewardKey]).toBeTruthy()
  })
