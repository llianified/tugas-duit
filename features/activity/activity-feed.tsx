...
/**
 * Satu item umpan bergaya thread: kolom avatar di kiri atas, lalu garis penghubung
 * berbentuk L (`.thread-line`) yang turun dari bawah avatar dan menikung ke isi.
 *
 * Angka geometrinya saling terikat dan tidak boleh diubah sendiri-sendiri:
 * avatar `size-10` (2.5rem) berarti sumbu tengahnya di 1.25rem, jadi blok isi
 * digeser `ml-5` supaya garis vertikalnya jatuh tepat di tengah avatar. `pl-5`
 * di sisi lain adalah 0.75rem lebar tikungan `.thread-line` + 0.5rem jarak baca.
 *
 * Nilainya pindah ke dalam blok isi, bukan menempel di tepi kanan seperti baris
 * `DataListRow`: pada 384px tepi kanan sudah dipakai waktu, dan menaruh keduanya
 * di sana memotong nama orangnya. Ini juga yang dilakukan fomo — harga duduk di
 * badan pesan, bukan di kolom kanan.
 */
function FeedItem({ entry, showDivider }: { entry: ActivityEntry; showDivider: boolean }) {
  ...
  return (
    ...
    >
      ...
      {/* Tanpa margin atas: tepi atas blok ini harus persis tepi bawah avatar,
      karena `.thread-line::before` menggambar garisnya dari `top: 0`. `pt-2`
      memberi tinggi pada bagian vertikalnya — tikungannya sendiri jatuh
      `0.5rem` di atas tepi bawah blok, jadi ia mendarat di tengah baris teks. */}
