...
- **`economy/ledger.ts` adalah satu-satunya jalan mengubah `balance_credits`.** Semua penambahan/
...
- `payout/payout.ts` (penarikan), `economy/quota.ts` (jaring anti-bot harian + plafon komisi),
  `economy/reward-pool.ts` (kolam reward yang menahan penghasilan), `task/challenge.ts` (siklus soal) memakai
  aturan dari `domain/`, bukan angka sendiri — kalau perlu ubah besaran, itu tugasnya
  `domain/economy/economy-config.ts`, bukan file di sini.
- `economy/reward-pool.ts` membaca kapasitas dengan query terpisah dari stoknya, dan itu **disengaja**:
...
- `platform/preview-db.ts`: mode dev tanpa Postgres nyata, pakai PGlite in-process. Jangan asumsikan
